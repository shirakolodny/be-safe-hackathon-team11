...
const StudentGame = ({ gameCode, studentName, onGameFinished }) => {
    ...
    const startGame = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/games/${gameCode}/start?username=${encodeURIComponent(studentName)}`
        );
        if (!res.ok) throw new Error('Failed to start game');

        const data = await res.json();

        // ❗ מתעלמים מ־finished של השרת
        setQuestionCount((data.answeredCount || 0) + 1);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFeedbackData({
        feedback: data.feedback,
        nextQ: data.nextQuestion,
        finished: isLastQuestion, // ✅ סיום רק אחרי 30
      });
    } catch (err) {
      console.error('Error submitting answer:', err);
    } finally {
  ...
  return (
    <Paper
      elevation={3}
      sx={{ p: 4, maxWidth: 700, mx: 'auto', mt: 4, minHeight: '400px' }}
      dir="rtl"
    >
      {/* Progress */}
      <Box sx={{ mb: 4 }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
          שאלה {questionCount} מתוך {TOTAL_QUESTIONS}
        </Typography>
        <LinearProgress
          variant="determinate"
          value={progress}
        ...
        <Fade in>
          <Box>
            ...
            <Button
              ...
              fullWidth
            >
              {loading ? <CircularProgress size={24} /> : 'שלח תשובה'}
            </Button>
          </Box>
        </Fade>
      )}
        ...
        <Fade in>
          <Box>
            ...
            <Button variant="primary" onClick={handleContinue} fullWidth>
              {questionCount >= TOTAL_QUESTIONS ? 'סיום המשחק' : 'המשך לשאלה הבאה'}
            </Button>
          </Box>
        </Fade>
      )}
